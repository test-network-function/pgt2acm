...
The goal of the pgt2acm tool is to ease the conversion between
[Policy Generator Templates](https://github.com/openshift-kni/cnf-features-deploy/tree/master/ztp/policygenerator)
and
[ACM Generator Templates](https://github.com/open-cluster-management-io/policy-generator-plugin)
...
``` default
 KUSTOMIZE_PLUGIN_HOME=$(pwd)/../../pgt2acm/kustomize pgt2acm -i
 mydir/policygentemplates -o mydir/acmgentemplates -s
...
- a working gitopts repository. For instance, consider the following directory
  structure:
...
not found, pgt2acm displays a error similar to (PtpConfig resource with slave1
name and openshift-ptp was not found in source-crs):

``` default
Could not convert PGT files, err: failed to convert PGT to ACMGen, err=could not
render patches in manifest, err:failed to process the manifest at
"acmgentemplates/source-crs/PtpConfigSlaveCustom-MCP-master.yaml": failed to
...
`KUSTOMIZE_PLUGIN_HOME=$(pwd)/../../pgt2acm-new/kustomize` : indicates the path
...
`-o policygentemplates` : the destination ACM Gen templates directory to be
created
...
for both PGT and ACM Gen templates. This option required a properly configured
Kustomize plugin home directory using the `KUSTOMIZE_PLUGIN_HOME` environment
variable.

A working directory including binaries for PGT and ACM Gen templates is provided
...
- `acmgen-out.yaml`: these are the policies generated by the translated ACM Gen
...
the new acmgentemplates deirectory to policygentemplates. ArgoCD would find the
...
mapped as follow between the PGT and ACM Gen templates
...
In ACM Gen Templates, the policies are listed under the policies field. The
files implementing the policies are listed under the policies->manifests
section. See the converted ACM Gen Template policy below. So in short in ACM
...
PGT and ACM Gen templates can apply patches to reference manifest using
Kustomize merge method. PGT can arbitrarily merge CRDs containing list of
objects but not ACM Gen. See the following issue in the
...
As a workaround, pgt2acm supports passing a open API schema that allows
kustomizing CRDs that contain list of objects such as in PtpConfig. In addition
to the schema, pgt2acmalso needs a list of CRD kinds that should be
pre-kustomized. Internally, pgt2acm will use the schema to Kustomize the
...
- If not using pre-rendering we en up with the following incorrect ACM Gen
  Template since
  [policy-generator-plugin](https://github.com/open-cluster-management-io/policy-generator-plugin)
  cannot Kustomize the PtpConfig CRD:
  [policies-acmGen.yaml](./docs/examples/policies-acmGen.yaml)
...
`master` or `worker` string when the PGT manifest is converted to ACM Gen.
pgt2acm generates new versions of manifests depending of whether they are used
...
generated for the ACM Gen named source-crs/`PtpConfigSlave`-MCP-worker.yaml. The
new path is properly referenced in the ACM Gen template.
